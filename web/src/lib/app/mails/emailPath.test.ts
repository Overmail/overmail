...
test("the id alone for a mail nothing knows the subject of", () => {
    expect(emailPath(id)).toBe(`/emails/${bare}`);
    expect(emailPath(id, null)).toBe(`/emails/${bare}`);
...
test("the subject goes in front, spaces as hyphens", () => {
    expect(emailPath(id, "Termin Donnerstag")).toBe(`/emails/Termin-Donnerstag-${bare}`);
    // Runs of whitespace are one hyphen, and none is left at either end.
    expect(emailPath(id, "  Deine   Bestellung\nist unterwegs  ")).toBe(
        `/emails/Deine-Bestellung-ist-unterwegs-${bare}`
    );
});

test("64 characters of it at most", () => {
    const subject = "A".repeat(70);
    expect(emailPath(id, subject)).toBe(`/emails/${"A".repeat(64)}-${bare}`);
});
...
test("a subject cut mid emoji is still a url", () => {
    ...
    expect(emailPath(id, subject)).toBe(`/emails/${"a".repeat(63)}%F0%9F%99%82-${bare}`);
});
...
test("the id comes back out of either form", () => {
    ...
    expect(parseEmailId(emailPath(id, "Re: a/b").replace("/emails/", ""))).toBe(id);
