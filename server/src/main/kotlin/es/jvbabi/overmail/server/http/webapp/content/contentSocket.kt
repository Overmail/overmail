 ...
 *
 * The mails on screen and nothing else. Where a mail *sits* is a listing's question, and the
 * listing socket answers it -- it reads the index again for itself when the mailbox moves, so
        ...
        clientWebSocket {
            ...
            launch {
                mailNotifier.subscribe(user.id.value)
                    ...
                    .collect { event ->
                        when (event) {
                            // Whatever it was: this socket sends the mail again. Whether it also
                            // moved a listing is the listing socket's business, not this one's.
